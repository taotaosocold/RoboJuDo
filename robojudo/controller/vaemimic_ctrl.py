...
class VaeMimicCtrl(Controller):
    cfg_ctrl: VaeMimicCtrlCfg
    env: Environment

    def __init__(self, cfg_ctrl: VaeMimicCtrlCfg, env, device="cpu"):
        ...
        code_data = np.load(cfg_ctrl.code_path, allow_pickle=True)
        self.code_book = code_data["codebook"]
        self.code_idx = code_data[cfg_ctrl.code_key]
        self.reset()
    ...
    @property
    def get_code_vector(self) -> np.ndarray:
        return self.code_book[self.code_idx[self.timestep]].flatten()

    def reset(self):
        self.timestep = 0
        self.pbar = ProgressBar(f"VaeMimicCtrl {self.cfg_ctrl.motion_name}", self.motion.time_step_total)
...
if __name__ == "__main__":
    ...
    ctrl = VaeMimicCtrl(cfg_ctrl=G1BeyondmimicCtrlCfg(), env=env)
