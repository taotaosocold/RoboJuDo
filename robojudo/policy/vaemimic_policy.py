...
from robojudo.utils.util_func import matrix_from_quat, subtract_frame_transforms, quat_apply_inverse

logger = logging.getLogger(__name__)


@policy_registry.register
class VaeMimicPolicy(Policy):
    cfg_policy: VaeMimicPolicyCfg

    def __init__(self, cfg_policy: VaeMimicPolicyCfg, device):
    ...
    def _prepare_policy(self):
        ...
        code_shape = self.session.get_inputs()[1].shape
        obs = np.zeros(obs_shape[1], dtype=np.float32)
        code = np.zeros(code_shape[1], dtype=np.float32)
        self.get_action(obs, code)
    ...
    def _get_command(self, env_data, ctrl_data):
        if not self.use_motion_from_model:
            assert "VaeMimicCtrl" in ctrl_data, "VaeMimicCtrl not found in ctrl_data"
            command = ctrl_data.get("VaeMimicCtrl")
    ...
    def get_observation(self, env_data, ctrl_data):
        ...
        command, robot_anchor_pos_w, robot_anchor_quat_w, anchor_pos_w, anchor_quat_w, code, hand_pose = self._get_command(
            env_data, ctrl_data
        )
        ...
        projected_gravity = quat_apply_inverse(robot_anchor_quat_w[[3, 0, 1, 2]], np.array([0., 0., -1.], dtype=np.float32))
    ...
    def get_action(self, obs: np.ndarray, code: np.ndarray) -> np.ndarray:
        ort_inputs = {
            "obs": np.expand_dims(obs, axis=0).astype(np.float32),
            "code": np.expand_dims(code, axis=0).astype(np.float32)
        }
