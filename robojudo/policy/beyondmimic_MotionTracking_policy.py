...
from robojudo.utils.util_func import matrix_from_quat, subtract_frame_transforms, quat_conjugate_np, quat_mul, quat_rotate_inverse_np, my_quat_rotate_np, calc_heading_quat_inv_np, yaw_quat_np
from collections import deque
logger = logging.getLogger(__name__)


@policy_registry.register
class BeyondMimicMotionTrackingPolicy(Policy):
    ...
    def __init__(self, cfg_policy: BeyondMimicPolicyCfg, device):
        ...
        self.robot_body_indexes = [0, 2, 4, 6, 8, 10, 12, 13, 15, 17, 18, 20, 22, 23]
        ...
        for _ in range(self.history_length):
            self.proprio_history_buffer.append(np.zeros(self.proprio_dim, dtype=np.float32))
    ...
    def _prepare_policy(self):
        ...
        obs_dict = {
            "policy_proprio_history": dummy_proprio,
            "future_motion": dummy_future,
        }
    ...
    def _get_command(self, env_data, ctrl_data):
        if not self.use_motion_from_model:
            assert "BeyondMimicMotionTrackingCtrl" in ctrl_data, "BeyondMimicMotionTrackingCtrl not found in ctrl_data"
            command = ctrl_data.get("BeyondMimicMotionTrackingCtrl")
            ...
            return (
                ...
                command.motion_anchor_pos_w,
                command.motion_anchor_quat_w,
                command.fk_info,
    ...
    def get_future_obs(self, future_motion, anchor_pos_w, anchor_quat_w):
        T, B, _ = future_motion["body_pos_w"].shape
        future_body_pos_w = future_motion["body_pos_w"]
        future_body_quat_w = future_motion["body_quat_w"]
        future_body_lin_vel_w = future_motion["body_lin_vel_w"]
        future_body_ang_vel_w = future_motion["body_ang_vel_w"]
        future_joint_pos = future_motion["joint_pos"]
        future_joint_vel = future_motion["joint_vel"]

        anchor_quat_inv = yaw_quat_np(anchor_quat_w)
        anchor_quat_inv = quat_conjugate_np(anchor_quat_inv)
        anchor_quat_inv_exp = np.tile(anchor_quat_inv, (T * B, 1))
        future_body_pos_r = future_body_pos_w - anchor_pos_w[np.newaxis, np.newaxis, :]
        future_body_pos_r = my_quat_rotate_np(anchor_quat_inv_exp, future_body_pos_r.reshape(-1, 3)).reshape(T, B, 3)        
        future_body_quat_r = quat_mul(anchor_quat_inv_exp, future_body_quat_w.reshape(-1, 4))
        future_body_ori_r = matrix_from_quat(future_body_quat_r)[:, :, :2].reshape(T, B, 6)

        future_body_lin_vel_r = my_quat_rotate_np(anchor_quat_inv_exp, future_body_lin_vel_w.reshape(-1, 3)).reshape(T, B, 3)
        future_body_ang_vel_r = my_quat_rotate_np(anchor_quat_inv_exp, future_body_ang_vel_w.reshape(-1, 3)).reshape(T, B, 3)
        
        body_features = np.concatenate([
            future_body_pos_r.reshape(T, -1),      # 所有body的Pos (B*3)
            future_body_ori_r.reshape(T, -1),      # 所有body的Ori (B*6)
            future_body_lin_vel_r.reshape(T, -1),  # 所有body的LinVel (B*3)
            future_body_ang_vel_r.reshape(T, -1),  # 所有body的AngVel (B*3)
        ], axis=-1)  # [T, B*15]

        future_obs = np.concatenate([body_features, future_joint_pos, future_joint_vel], axis=-1)
    ...
    def _compute_robot_body_state(self, fk_info, robot_anchor_pos_w, robot_anchor_quat_w):
        ...
        anchor_quat_inv = quat_conjugate_np(robot_anchor_quat_w)
    ...
    def get_observation(self, env_data, ctrl_data):
        ...
        future_motion = ctrl_data.get("BeyondMimicMotionTrackingCtrl").get("future_motion", None)
        ...
        # obs_base_lin_vel = quat_rotate_inverse_np(robot_anchor_quat_w, lin_vel)
        # obs_base_ang_vel = quat_rotate_inverse_np(robot_anchor_quat_w, ang_vel)
        ...
        self.proprio_history_buffer.append(obs_prop)
        while len(self.proprio_history_buffer) < self.history_length:
            self.proprio_history_buffer.appendleft(obs_prop)
        future_obs = self.get_future_obs(future_motion, motion_anchor_pos_w, motion_anchor_quat_w)
        np.set_printoptions(threshold=np.inf, linewidth=np.inf)
        obs_dict = {
            "policy_proprio_history": np.array(self.proprio_history_buffer)[np.newaxis, :],
            "future_motion": future_obs[np.newaxis, :],
        }
        extras = {
            "pos": pos,
            "ori": ori,
